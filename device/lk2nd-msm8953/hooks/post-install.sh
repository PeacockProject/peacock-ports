#!/bin/sh
# lk2nd-msm8953 post-install: flash the lk2nd bootloader to the boot partition.
#
...
img="${FEATHER_PREFIX:-}/usr/share/peacock/bootloaders/lk2nd-msm8953.img"
[ -f "$img" ] || img="/usr/share/peacock/bootloaders/lk2nd-msm8953.img"

dev="${PEACOCK_BOOT_DEV:-}"
if [ -z "$dev" ] || [ ! -b "$dev" ]; then
	echo "lk2nd-msm8953: no flash target (PEACOCK_BOOT_DEV unset) — image staged, not flashing"
	exit 0
fi
[ -f "$img" ] || { echo "lk2nd-msm8953: image not found: $img" >&2; exit 1; }

echo "lk2nd-msm8953: flashing $img -> $dev"
...
$DD if="$img" of="$dev" bs=4096 conv=fsync 2>/dev/null || { echo "lk2nd-msm8953: dd failed" >&2; exit 1; }
sync
echo "lk2nd-msm8953: flashed lk2nd to $dev"
