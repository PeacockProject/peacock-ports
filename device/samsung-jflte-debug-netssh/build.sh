# shellcheck shell=sh
# samsung-jflte-debug-netssh — stage the debug USB RNDIS + DHCP + SSH helper
# scripts, their config, and the OpenRC services into $pkgdir. No source/compile;
# prepare() is a no-op.
...
find_usb_base() {
    for p in /sys/class/android_usb/android0 /sys/devices/virtual/android_usb/android0 /sys/devices/virtual/usb_composite/*; do
        if [ -d "$p" ] && [ -e "$p/enable" ] && [ -e "$p/functions" ]; then
            echo "$p"
            return 0
        fi
    done
...
if [ -z "$usb_base" ]; then
    echo "rndis: android_usb path missing" >> "$logf"
    exit 0
fi
...
sys_write "$usb_base/iSerial" "PEACOCK-JFLTE-DEBUG" || true
...
#!/sbin/openrc-run
description="Peacock debug USB RNDIS network (PRP-style)"
...
echo "peacock-debug-sshd: sshd binary not found" >&2
exit 127
EOF_SSHD_WRAPPER
...
command_args="-D -f /etc/ssh/sshd_config_peacock_debug -E /var/log/peacock-debug-sshd.log"
