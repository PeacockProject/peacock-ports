#!/bin/sh
# Kali locale stage. Kali inherits Debian's locale machinery: no locales are generated out of the
# box and the generator ships in the `locales` package, so: make sure locale-gen exists, list the
# wanted locale in /etc/locale.gen, generate it, and record it in /etc/default/locale — the
# system-wide locale file PAM reads on every login. /etc/locale.conf (the Arch file) is NOT it; the
# container image even ships /etc/default/locale as a symlink *to* /etc/locale.conf, so replace the
# link with a real file and keep locale.conf in step for systemd/localectl.
set -u
