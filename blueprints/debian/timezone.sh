#!/bin/sh
# Debian timezone stage. Debian records the zone TWICE: the /etc/localtime symlink (what glibc
# reads) and /etc/timezone (the textual name tzdata's debconf owns). Setting only the symlink works
# until the next tzdata upgrade, which reconfigures from /etc/timezone and silently puts the clock
# back — so write both, then let tzdata reconcile them itself.
set -u
